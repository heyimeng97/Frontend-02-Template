function layout(element) {
    ...
    if(!style[crossSize]){
        ...
        for(let i=0; i< flexLines.length; i++){
            elementStyle[crossSzie] = elementStyle[crossSize] + flexLines[i].crossSpace;
        }else{
    ...
    function getStyle(element) {
        if (!element.tempStyle) {
            element.tempStyle = {};
        }

        for (let prop in element.computedStyle) {
            let p = element.computedStyle.value;
            element.tempStyle[prop] = element.computedStyle[prop].value;

            if (element.tempStyle[prop].toString().match(/px$/)) {
                element.tempStyle[prop] = parseInt(element.tempStyle[prop])
            }
            if (element.tempStyle[prop].toString().match(/^[0-9\.]+$/)) {
                element.tempStyle[prop] = parseInt(element.tempStyle[prop]);
            }

            return element.tempStyle;
        }
    }